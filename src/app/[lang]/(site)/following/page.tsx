...
export default async function FollowingPage({
  ...
  const stores: Store[] = ((data ?? []) as unknown as FollowRow[])
    .map((f) => f.stores)
    .filter((s): s is NonNullable<FollowRow["stores"]> => Boolean(s))
    .map((s) => ({
      id: s.id,
      ...
      <Container>
        ...
        {stores.length ? (
          ...
          <div className="mt-8 rounded-2xl border border-dashed border-border py-16 text-center">
            ...
            <p className="mt-4 text-muted-foreground">{dict.following.empty}</p>
          </div>
        )}
      </Container>
    </div>
