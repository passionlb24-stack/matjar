...
| Feature graphic | 1024×500 PNG | `docs/store-assets/feature-graphic-{dark,light}.png` — **two ready, pick one** |
| Phone screenshots | ≥2, min 320px side | capture from the app |

The feature graphics are generated by `scripts/make-feature-graphic.mjs`, not
drawn by hand, so editing the copy means editing the script and re-running
`node scripts/make-feature-graphic.mjs`. It embeds the real `icon-512.png` and
uses the app's own sector tints, and it **refuses to write a file whose content
comes within 8% of any edge** — Play crops this graphic differently across its
surfaces, and a half-eaten word is the usual result of designing it by eye.
Decorative background is expected to bleed; the check measures the DOM boxes of
the actual content, which is the only way to tell a deliberate bleed from a
clipped word.
